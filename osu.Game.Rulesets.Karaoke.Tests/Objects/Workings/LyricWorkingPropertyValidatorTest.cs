...
public class LyricWorkingPropertyValidatorTest : HitObjectWorkingPropertyValidatorTest<Lyric, LyricWorkingProperty>
{
    ...
    public void TestSingers()
    {
        ...
        Assert.That((Action)(() => lyric.SingerIds = new List<ElementId>()), Throws.Nothing);
        AssetIsValid(lyric, LyricWorkingProperty.Singers, true);

        // should be invalid if assign the singer.
        Assert.That((Action)(() => lyric.SingerIds.Add(singerId1)), Throws.Nothing);
        AssetIsValid(lyric, LyricWorkingProperty.Singers, false);

        // should be valid again if remove the singer
        Assert.That((Action)(() => lyric.SingerIds.Remove(singerId1)), Throws.Nothing);
        AssetIsValid(lyric, LyricWorkingProperty.Singers, true);

        // should be matched if include all singers
        Assert.That((Action)(() => lyric.SingerIds = new List<ElementId> { singerId1, singerId2, singerId3 }), Throws.Nothing);
        Assert.That((Action)(() => lyric.Singers = new Dictionary<Singer, SingerState[]>
        {
        ...
        // should works even id is not by order.
        Assert.That((Action)(() => lyric.SingerIds = new List<ElementId> { singerId3, singerId2, singerId1 }), Throws.Nothing);
        Assert.That((Action)(() => lyric.Singers = new Dictionary<Singer, SingerState[]>
        {
            { new Singer().ChangeId(singerId1), Array.Empty<SingerState>() },
            { new Singer().ChangeId(singerId2), Array.Empty<SingerState>() },
        ...
        Assert.That((Action)(() => lyric.SingerIds = new List<ElementId> { singerId1, singerId1, singerId1 }), Throws.Nothing);
        Assert.That((Action)(() => lyric.Singers = new Dictionary<Singer, SingerState[]>
        {
            { new Singer().ChangeId(singerId1), Array.Empty<SingerState>() },
        }), Throws.Nothing);
        ...
        // should works if id is duplicated
        Assert.That((Action)(() => lyric.SingerIds = new List<ElementId> { singerId1 }), Throws.Nothing);
        Assert.That((Action)(() => lyric.Singers = new Dictionary<Singer, SingerState[]>
        {
            { new Singer().ChangeId(singerId1), Array.Empty<SingerState>() },
            { new Singer().ChangeId(singerId1), Array.Empty<SingerState>() },
    ...
    public void TestPage()
    {
        ...
        Assert.That((Action)(() => lyric.PageIndex = 1), Throws.Nothing);
    ...
    public void TestReferenceLyric()
    {
        ...
        // should be invalid if change the reference lyric id.
        Assert.That((Action)(() => lyric.ReferenceLyricId = TestCaseElementIdHelper.CreateElementIdByNumber(2)), Throws.Nothing);
        AssetIsValid(lyric, LyricWorkingProperty.ReferenceLyric, false);

        // should be valid again if assign the reference lyric to the matched lyric.
        Assert.That((Action)(() => lyric.ReferenceLyric = new Lyric().ChangeId(2)), Throws.Nothing);
        AssetIsValid(lyric, LyricWorkingProperty.ReferenceLyric, true);

        // should throw the exception if assign the working reference lyric to the unmatched reference lyric id.
        Assert.That((Action)(() => lyric.ReferenceLyric = new Lyric().ChangeId(3)), Throws.TypeOf<InvalidWorkingPropertyAssignException>());
        Assert.That((Action)(() => lyric.ReferenceLyric = null), Throws.TypeOf<InvalidWorkingPropertyAssignException>());
    }
