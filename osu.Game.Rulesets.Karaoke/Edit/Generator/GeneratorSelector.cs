...
{
    public abstract class GeneratorSelector<TGenerator, TConfig> where TGenerator: class where TConfig: new()
    {
        protected Dictionary<CultureInfo, Lazy<TGenerator>> Generator { get; private set; }

        protected void RegistGenerator<T, TC>(CultureInfo info) where T : TGenerator where TC : TConfig
        {
